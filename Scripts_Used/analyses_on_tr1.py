...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 25 14:02:57 2020
...
for UPDATED_file_tr in UPDATED_files_tr: 
    ...
    df_training_1 = df_training_1.loc[df_tr['resp_total_corr'] == 1] 
    Total_resp_Seq_1 = df_training_1["resp_total_corr"].sum()
    
    df_training_1_wo = df_exp_fil_trials.loc[df_tr['repeat_training_loop1.thisRepN'] >= 0]
    df_training_1_wo = df_training_1_wo.loc[df_tr['resp_total_corr'] == 1] 
    df_training_1_wo = df_training_1_wo.drop(df_training_1_wo[df_training_1_wo['trial_type'] == "filler"].index)
    Corr_S1_Tot_wo = df_training_1_wo["resp_total_corr"].sum() #number of correct sequences (without fillers) - all loops

    df_tr_1_tot.append(df_training_1_wo)

  
df_tr_1_tot = pd.concat(df_tr_1_tot)
df_tr_1_tot = df_tr_1_tot[["participant", "resp1.rt", "resp_total_time", "conditions", 
                           "repeat_training_loop1.thisRepN"]] 

df_tr_1_tot = df_tr_1_tot.rename(columns = {'participant': 'Id', 'resp1.rt': 'OT', 
                                              'resp_total_time': 'RT', 'conditions': 'condition',
                                              "repeat_training_loop1.thisRepN": "loop"}, inplace = False)

 
df_tr_1_tot.to_csv("/data/pt_02312/results/results_training_1.csv", index = False, header=True)

result_total = df_tr_1_tot
